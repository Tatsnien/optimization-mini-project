...
class Ant:
    ...
    def generate_probabilities(self):
        ...
        for ver in self.unvisited:
            sum += (self.aco.pheromone[self.current][ver]**self.aco.alpha) * (self.compute_eta(self.current, ver)**self.aco.beta)
        for ver in self.unvisited:
            p[ver] = (self.aco.pheromone[self.current][ver]**self.aco.alpha) * (self.compute_eta(self.current, ver)**self.aco.beta) / sum
        return p
...
if __name__ == "__main__":
    ...
    nbAnts = min(M + 1, 50)
    aco = ACO(d, Q, q, nbAnts=nbAnts, generations=20, alpha=2, beta=5)
    aco.solve()